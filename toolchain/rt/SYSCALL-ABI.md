...
| `$3D` | `sys_wait4` | `R0`=pid (`-1`=any), `R1`=`int *status`, `R2`=options, `R3`=rusage | `RV`=reaped pid or `-errno`; options honors `WNOHANG` (`RV`=0 when nothing matches) and `WUNTRACED` (also matches a stopped child, reported once and not reaped) since maize-361 |
