...
## The erratum trigger admits a silence

**Status: SETTLED. 2026-08-12, under the rule above.** The versioning chapter defined an erratum candidate as a passage that is ambiguous, self-contradictory, or wrong about what the reference implementation and the conformance suite already agree on. That disjunction has no clause for a passage that simply says nothing where the specification promises to leave nothing unsaid, and the first issuance ran straight into two such passages. Three passages state that definition, and all three now add "or silent where this specification promises no silence": the trigger sentence in `versioning.md`, the sentence mirroring it in `errata.md`, and the front page's restatement in `README.md`. The bound below the trigger is untouched, and it still holds for a silence by construction, since no conformance test could have pinned an answer that did not exist.

Widening the trigger is a direct edit to ratified text rather than an erratum, by the observable-behavior test recorded above. Which passages the mechanism may correct is not something a conformance binary, a machine, or a program the machine runs can observe. Editing the definition changes nothing an implementer could detect, so it consumes no version component and is recorded here instead of in the errata log.

Erratum `2.0.1` is the first issuance under the widened definition. Two of its three entries, the faulting data address of an interrupted `block_copy` and the auxiliary word for an invalid bitfield immediate pair, correct silences that an operator ruling settled, and each entry says so. The third, the block-memory completion state, is compelled by the frozen text alone and would have qualified under the old definition unchanged.

One question this raises is deliberately left open. Whether a ruling that fills a silence may oblige the reference implementation to change its behaviour has no case to test it against yet, since neither ruled correction in `2.0.1` obliges one, and a governance rule written with no case in front of it is the kind that turns out to have a hole. Whichever erratum first meets that case settles it. That case arrived with erratum `2.0.2`, and the section below records the settlement.

## An erratum may move the reference implementation where the text was silent

**Status: RULED. Operator, 2026-08-13, on card maize-452.** The question the section above left open now has a case in front of it. Erratum `2.0.2` defines the class contract version, a field the device chapter named in the second quarter-word of the identification word every class's port at offset 0 reads and then said nothing further about, and the reference implementation consequently reports `$0001` where it reported `$0100`. The ruling is that a correction filling a silence may oblige the reference implementation to change its behavior, where the behavior it changes was not constrained by the prior text.

The reasoning turns on what the bound in the versioning chapter actually protects. That bound says an erratum changes no conformance test's expected result, and that a correction which would change what a conforming machine does is not an erratum candidate. Both clauses presuppose a specified behavior to change. Where the specification was silent, every value conformed, so a machine reporting `$0100` and a machine reporting `$0001` were equally correct. The `$0100` the reference implementation shipped was an implementation choice made in the absence of an architectural one, and the constant's own name recorded it as such. Replacing an unconstrained choice with a constrained one changes what that implementation does while changing nothing the specification requires of a machine that conforms to the corrected text; the set of conforming machines was wider before and is narrower now.

The limit carries more weight here than the permission does. This ruling does not license an erratum to change a behavior the specification constrained, however lightly or incidentally it did so. The test is whether the prior text bore on the behavior at all, and it is emphatically not whether a conformance test happened to assert it. A behavior that is specified but untested lies outside this rule, and a behavior that is unspecified and therefore untestable lies inside it. Anyone reaching for this precedent has to show the silence rather than the absence of a test.

One consequence is worth stating plainly instead of leaving someone to discover it. An erratum issued under this rule narrows the set of conforming machines, since a machine built to the earlier text and reporting `$0100` conformed then and does not conform now. That is acceptable in this instance because no such machine exists, and because the alternative is a named field left undefined for the life of the architecture. It would not be acceptable for a silence discovered after third-party implementations exist, and a future erratum in that position weighs the question again rather than citing this ruling as having settled it.
