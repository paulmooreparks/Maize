...
- Decimal marking: **RULED, always-explicit bases are mandatory. Operator, 2026-08-11, overruling the bare-decimal recommendation.** Every numeric literal names its base: `#` decimal, `$` hex, `%` binary, with no bare numbers accepted. The operator's grounds: no accidents. A base is never inferred by the lexer or by a reader, which is the v1 discipline carried forward unchanged, and it is a correctness stance rather than a style preference.
