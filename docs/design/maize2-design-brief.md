...
The vocabulary is redrawn with the ISA: dotted sub-registers become width modifiers on operations plus the extract/insert forms, and the CISC addressing forms go with the CISC. Two conventions are consciously re-decided rather than inherited, and are Phase 1 decisions: operand order (v1's source-to-destination comma-free style versus the destination-first order every RISC-lineage assembler uses) and whether decimal stays marked with `#` or becomes the bare default.
...
- D11: the two assembler conventions (operand order; decimal marking).
