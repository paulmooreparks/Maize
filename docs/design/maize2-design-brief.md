...
Registers are full-width and plain. Width lives on the operation as a modifier, with one rule everywhere, spelled in Maize's own literal size vocabulary (operator ruling 2026-08-11, and v1's sub-register letters already had it): a word is the native 64 bits, so `.b` is a byte (8), `.q` a quarter-word (16), `.h` a half-word (32), and a bare mnemonic means the full word. This is deliberately not the Intel or RISC-V frozen convention where "word" means 16 or 32. Loads state their extension explicitly (the LDZ/LDS pair as modifiers, in the shape of `load.zb` and `load.sb`, so the pair is regular by construction and the v1 asymmetry cannot recur). The half-word ALU forms cover C's `int` semantics with a defined result in the full register.
...
The vocabulary is redrawn with the ISA, and the dotted notation is repartitioned rather than removed: width modifiers ride memory operations, positional dotted register operands ride the extract and insert forms, and the CISC addressing forms go with the CISC. Four conventions are consciously re-decided rather than inherited, and are Phase 1 decisions: operand order (v1's source-to-destination comma-free style versus the destination-first order every RISC-lineage assembler uses); whether decimal stays marked with `#` or becomes the bare default; mnemonic register (terse abbreviations in the v1 style versus full lowercase words in the LLVM-IR and WebAssembly-text style, which is the direction every machine-first text format designed after the teletype era chose, raised by the operator 2026-07-24 on the grounds that compilers and models emit assembly and humans mostly read it while debugging); and case policy. Whatever the mnemonic decision, the disassembler and any formatter emit exactly one canonical name per operation. Whether a blessed short-alias table exists alongside the canonical names (the PowerShell Write-Host/echo model, operator-raised 2026-07-24) is an open sub-question, with the tradeoffs on record: aliases in PowerShell serve an interactive prompt that assembly does not have, and that ecosystem needed formatter and linter policing to keep aliases out of committed code, but the mitigations are known (canonical-only tool output, normalize-on-format). The recorded recommendation is one canonical name at v2 launch, with the alias table deferred, because it is a purely additive assembler-layer feature with no ISA content and can be added later from felt friction without foreclosing anything. Full-word length is a cold-tail concern only: the hot instructions are short as words (load, store, add, jump), and the wordiest realistic mnemonics (in the shape of fused_multiply_subtract, tlb_invalidate_all, or a future atomic_compare_exchange) belong to rare operations that can afford the length, with pragmatic abbreviation at the extreme tail (the WebAssembly cmpxchg precedent) a legitimate tool.

The separator policy is settled in direction (operator preference, 2026-07-24, no parser objection found): compound names spell their words with underscores (compare_exchange, tlb_invalidate_all), which matches the WebAssembly text format's own convention (dot as namespace, underscores in words) and makes every mnemonic a valid C identifier, so the spec tables, the tokenizer, and the VM dispatch enum can share literal names under the grammar-mirrors-tokenizer discipline. The dot is reserved for operand-level structure and never appears inside a name. Specifically it survives in the positional register suffix (r3.b5), where it earns its keep for a genuine lexer reason: labels live in the underscore alphabet (v1's own loop_condition and hw_string), so an underscore-spelled positional form would need a 448-entry reserved-name list to stay unambiguous, while the dot, excluded from the identifier alphabet, makes positional forms structurally distinct from labels at no cost. The one dot the operator left open inside a mnemonic is the length specifier, and only that (operator clarification, same date): `load.zb` versus `load_zb` is a D6/D11 sub-choice, but everything else that compounds a name, including condition families, spells with underscores (`jump_zero`, `branch_eq`, `atomic_compare_exchange`), never with dots. The tradeoff on the surviving sub-choice: a flat underscore name keeps the assembler to a single token class with no modifier grammar, while a dotted length specifier makes the width family visible as a family and keeps the name table smaller, at the cost of the dot doing double duty between operand structure and this one name position. The mnemonic decision interacts with D3 (a `jump.zero` spelling presumes flags) and with the width-modifier scheme (full words allow the extension pair to fold into the modifier, in the shape of `load.zb` and `load.sb`). Width letters follow the literal size vocabulary throughout: byte, quarter-word, half-word, word.
...
- D5: precise width-modifier semantics (what a half-word `.h` operation writes into the upper 32 bits, and whether that matches x86-64 zeroing or something else).
...
- D11: the assembler conventions (operand order; decimal marking; mnemonic register, terse versus full words; whether a blessed short-alias table accompanies the canonical names or arrives later, if ever; case policy; and the length-specifier spelling sub-choice, load_zb versus load.zb, under the settled rule that underscores spell every other compound name and the dot is otherwise reserved for operand structure).
...
The encoding gate is satisfied (2026-08-11): the v1 JIT characterization exists in full, so Phase 1 may run all thirteen decisions from day one. The evidence lives on the maize-343 card (the debug matrix plus the release-build addendum measured with scripts/jit-characterize.sh) and the maize-354 card (the attribution of the paged-versus-bare gap). The findings the encoding decisions should absorb: the JIT reaches roughly 99% coverage and 10x-plus speedups, so the encoding is designed for translation first; the dominant memory cost is translation structure, not decode arithmetic (single-entry fast-page misses bailing to a C++ slow path were the gap, and multi-way inline TLB probing closed it to a 1.7x residual), which endorses the flat-reservation-plus-inline-TLB memory design; and syscall-bound code gains only about 1.8x because SYS ends a compiled block, which is direct pressure on D8 and D13 to make trap entry and the syscall shape JIT-friendly. Everything in the hybrid plan after Phase 1 is gated on ratified decisions, not on calendar.
