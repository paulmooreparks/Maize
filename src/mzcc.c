...
int resolve_toolchain(const char *preset) {
    ...
    free(mazm_base);
    if (!MAZM) { fprintf(stderr, "mzcc: mazm not found in %s; run scripts/install-mzasm.sh --with-c-toolchain (or run-tests.sh) first.\n", BUILD_DIR); return 2; }
    free(MAIZE);
    char *maize_base = path_join(BUILD_DIR, "maize");
    MAIZE = resolve_exe(maize_base);
    free(maize_base);
    if (!MAIZE) { fprintf(stderr, "mzcc: maize not found in %s; run scripts/install-mzasm.sh --with-c-toolchain (or run-tests.sh) first.\n", BUILD_DIR); return 2; }
    free(MZLD);
    char *mzld_base = path_join(BUILD_DIR, "mzld");
    MZLD = resolve_exe(mzld_base);
    free(mzld_base);
    if (!MZLD) { fprintf(stderr, "mzcc: mzld not found in %s; run scripts/install-mzasm.sh --with-c-toolchain (or run-tests.sh) first.\n", BUILD_DIR); return 2; }
