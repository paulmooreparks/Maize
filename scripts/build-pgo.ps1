...
if (-not (Test-Path $BenchImage)) { Write-Error "BenchImage not found: $BenchImage"; exit 2 }
if (-not (Test-Path $Wad))        { Write-Error "Wad not found: $Wad"; exit 2 }
...
else { Write-Error 'cmake not found on PATH or at C:\Program Files\CMake\bin\cmake.exe.'; exit 2 }
...
if (-not (Test-Path $LlvmProfdata)) {
    Write-Error "llvm-profdata not found at $LlvmProfdata; run scripts/bootstrap-toolchain.ps1 first."
    exit 2
}
...
& $Cmake --preset $Preset "-DMAIZE_PGO=generate" "-DMAIZE_PGO_DIR=$PgoDir"
if ($LASTEXITCODE -ne 0) { Write-Error "configure (generate) failed (exit $LASTEXITCODE)."; exit 2 }

Write-Host "== Phase 1/4: building instrumented maize =="
& $Cmake --build $BuildDir --target maize
if ($LASTEXITCODE -ne 0) { Write-Error "build (generate) failed (exit $LASTEXITCODE)."; exit 2 }
...
for ($i = 1; $i -le $Runs; $i++) {
    ...
    if ($LASTEXITCODE -ne 0) { Write-Error "training run $i failed (exit $LASTEXITCODE)."; exit 2 }
}

$rawProfiles = @(Get-ChildItem -Path $PgoDir -Filter '*.profraw' -ErrorAction SilentlyContinue)
if ($rawProfiles.Count -eq 0) { Write-Error "no .profraw files produced under $PgoDir; instrumented maize.exe did not run cleanly."; exit 2 }
...
if ($LASTEXITCODE -ne 0 -or -not (Test-Path $merged)) { Write-Error "llvm-profdata merge failed (exit $LASTEXITCODE)."; exit 2 }

# --- Phase 4: rebuild optimized ------------------------------------------------------
Write-Host "== Phase 4/4: configuring '$Preset' with MAIZE_PGO=use =="
& $Cmake --preset $Preset "-DMAIZE_PGO=use" "-DMAIZE_PGO_DIR=$PgoDir"
if ($LASTEXITCODE -ne 0) { Write-Error "configure (use) failed (exit $LASTEXITCODE)."; exit 2 }

Write-Host "== Phase 4/4: building profile-guided maize + maizeg =="
& $Cmake --build $BuildDir --target maize maizeg
if ($LASTEXITCODE -ne 0) { Write-Error "build (use) failed (exit $LASTEXITCODE)."; exit 2 }
