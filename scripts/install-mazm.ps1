...
.DESCRIPTION
    ...
    $HOME\bin). Also refreshes the maize-cc.cmd Windows forwarder (the C-toolchain
...
# --- Windows forwarder: refresh <InstallDir>\maize-cc.cmd from the repo template ----
# maize-cc.cmd is the Windows entry point for the C toolchain: it wslpath-translates
# the source path and calls the WSL-side ~/bin/maize-cc (regenerated by
...
# the four .exe surfaces and structurally unable to go stale (maize-96 OQ2). The help
# text names the current .mzx linked image (the old copy still said the stale .bin).
$maizeCcCmd = @'
@echo off
rem maize-cc: compile a C source through the Maize C toolchain and run it.
rem cproc/qbe are POSIX-only, so the pipeline runs in WSL; this forwarder
rem translates the Windows path and invokes ~/bin/maize-cc inside WSL, which
rem execs scripts/cc-maize.sh (the single canonical C driver).
rem   maize-cc <file.c>          compile and run
rem   maize-cc <file.c> --emit   also leave <file>.mazm / <file>.mzx beside the source
rem   maize-cc --build           rebuild the cproc/qbe toolchain
setlocal enabledelayedexpansion
if "%~1"=="" (
  echo usage: maize-cc ^<file.c^> [--emit]   ^(also: maize-cc --build^)
  exit /b 2
)
if /I "%~1"=="--build" (
  wsl.exe -e bash -lc "~/bin/maize-cc --build"
  exit /b !errorlevel!
)
for /f "usebackq delims=" %%p in (`wsl.exe wslpath "%~f1"`) do set "WSLSRC=%%p"
wsl.exe -e bash -lc "~/bin/maize-cc '!WSLSRC!' %~2"
exit /b !errorlevel!
'@
$cmdPath = Join-Path $InstallDir 'maize-cc.cmd'
Set-Content -Path $cmdPath -Value $maizeCcCmd -Encoding Ascii
Write-Host "Refreshed $cmdPath (Windows forwarder to WSL maize-cc)."
