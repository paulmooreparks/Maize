...
- Signed and unsigned DIV/MOD variants both exist.
- ADC and SBB handle wide arithmetic.
- A wide multiply exposes the high half of the 128-bit product.
- The missing branch complements JGE, JLE, JBE, and JAE are in.
...
- A portable C++ toolchain with no MSVC dependency, using CMake, Ninja,
  ...
  Windows), driven from presets on the command line so that VS Code, Zed,
  Vim, or a bare terminal all work equally well.
- Assembler portability and diagnostics, with mazm building and running
  cleanly on every supported platform and producing useful errors on bad
  input.
...
- A string-output syscall and a shared test library, sparing test
  programs the hand-counting of string lengths.
- CI on every push across Linux and Windows.
- A fix for the Release-build undefined behavior in the register union.
...
- The assembler got cleaned up, including proper error reporting, though
  Unicode source files are still open and carry into Phase 2.
- mzdis disassembles.
...
- The per-instruction specification covers encoding, operation, flag
  ...
  interrupt model, and it runs to 21 chapters under `docs/spec/`.
- The versioning policy makes v1.0 a freeze, so binaries assembled
  against it run forever on any v1.x VM.
...
- The normative chapter, already written at `docs/spec/conformance.md`,
  with the executable suite still outstanding.
...
- The device set covers console, keyboard, framebuffer, timer, and a
  host-filesystem mount layer, all with interrupts.
...
- An LLM evaluation harness as a separate release and a second post,
  with the spec, a task suite, and a deterministic grader.

The evaluation harness is worth the separate release because Maize has
zero training-data presence, so it tests reasoning from a specification
rather than recall.
...
- A filesystem using a format that already exists, either ext2 or FAT,
  since formats get borrowed and never invented.
...
- A pointer device, queued and interrupt-driven, with absolute
  coordinates, buttons, and a wheel, being the last piece of input
