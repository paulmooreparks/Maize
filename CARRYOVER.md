...
- Infrastructure: pinned llvm-mingw bootstrap (no MSVC), CMake presets per platform, `scripts/install-mazm.{ps1,sh}` installs maize/mazm/mzld/mzdis to `~/bin` and refreshes the mzcc forwarder. mzcc and the whole guest C toolchain build and run natively on Windows via Git Bash, no WSL and no MSYS2 required (maize-257).
