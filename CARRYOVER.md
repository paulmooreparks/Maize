...
- Infrastructure: pinned llvm-mingw bootstrap (no MSVC), CMake presets per platform, `scripts/install-mzasm.{ps1,sh}` builds and installs maize/maizeg/mzvm/mzvmg/mazm/mzasm/mzld/mzdis/mzcc to `~/bin`. mzcc and the whole guest C toolchain build and run natively on Windows, no WSL and no MSYS2 required (maize-257); the only build step that still needs a POSIX shell (Git Bash) is compiling the vendored cproc/QBE via build-toolchain.sh (`mzcc --build`).
